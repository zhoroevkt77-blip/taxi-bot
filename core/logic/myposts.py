...
def pickup_start(messenger, msg, account, post_id):
    ...
    _say(messenger, msg, account, L(
        ...
        f"1️⃣ Ушул шилтемени жүргүнчүлөргө жөнөтүңүз:\n"
        f"{base}/pickup/{token}\n\n"
        f"Алар бир баскыч басып, турган жерин белгилейт.\n\n"
        f"2️⃣ Өзүңүздүн картаңыз — ким кайда тургандыгы, чогултуу ирети "
        f"жана навигатор:\n{base}/pickup/m/{mtoken}\n\n"
        f"Жүргүнчүгө мындай деп жазсаңыз болот:\n"
        f"<i>«Саламатсызбы! Мен айдоочумун. Ушул шилтемени басып, "
        f"«📍 Мен ушул жердемин» дегенди тандаңыз — сизди ошол жерден "
        f"алып кетем.»</i>\n\n"
        f"ℹ️ Толук нускама: 🆘 Жардам → ❓ Суроолор → "
        f"🗺 Жүргүнчүлөрдү чогултуу\n"
        f"<i>Локациялар 3 күндөн кийин өзү өчөт.</i>",
        f"🗺 <b>Сбор пассажиров</b>\n"
        f"{p['from_city']} ➡️ {p['to_city']}\n\n"
        f"1️⃣ Отправьте эту ссылку пассажирам:\n"
        f"{base}/pickup/{token}\n\n"
        f"Они одним нажатием отметят, где находятся.\n\n"
        f"2️⃣ Ваша карта — кто где стоит, порядок сбора и навигатор:\n"
        f"{base}/pickup/m/{mtoken}\n\n"
        f"Пассажиру можно написать так:\n"
        f"<i>«Здравствуйте! Я водитель. Откройте эту ссылку и нажмите "
        f"«📍 Я нахожусь здесь» — я заберу вас оттуда.»</i>\n\n"
        f"ℹ️ Полная инструкция: 🆘 Помощь → ❓ Вопросы → "
        f"🗺 Жүргүнчүлөрдү чогултуу\n"
        f"<i>Локации удаляются через 3 дня.</i>"), back_kb())
